...
const columns: ["money", "divorce", "love", "outdated", "unbound"] = [
  "money",
  "divorce",
  "love",
  "outdated",
...
const categories = {
  money: {
    name: "Hohe Kosten einer Hochzeit",
    color: "var(--color-primary)",
  },
  divorce: {
    name: "Risiko einer schwierigen / teuren Scheidung",
    color: "var(--color-secondary)",
  },
  love: {
    name: "Man kann auch so zusammenleben",
    color: "var(--color-accent)",
  },
  outdated: {
    name: "Heirat ist ein veraltetes Modell",
    color: "var(--color-info)",
  },
  unbound: {
    name: "Will persönliche Unabhängigkeit bewahren",
...
responses.value.forEach((entry) => {
  ageResponses[entry.age].push(entry.contra);
});
  ...
  if (ageRange.length > 0) {
    data.value.push({
      ...
      money:
        (ageRange.reduce(
          (a: number, b) =>
            b.includes("Hohe Kosten einer Hochzeit") ? a + 1 : a,
          0,
        ) /
          ageRange.length) *
        100,
      divorce:
        (ageRange.reduce(
          (a: number, b) =>
            b.includes("Risiko einer schwierigen / teuren Scheidung")
              ? a + 1
              : a,
          0,
        ) /
      ...
      love:
        (ageRange.reduce(
          (a: number, b) =>
            b.includes("Man kann auch so zusammenleben") ? a + 1 : a,
          0,
        ) /
          ageRange.length) *
        100,
      outdated:
        (ageRange.reduce(
          (a: number, b) =>
            b.includes("Heirat ist ein veraltetes Modell") ? a + 1 : a,
          0,
        ) /
          ageRange.length) *
        100,
      unbound:
        (ageRange.reduce(
          (a: number, b) =>
            b.includes("Will persönliche Unabhängigkeit bewahren") ? a + 1 : a,
          0,
        ) /
          ageRange.length) *
        100,
